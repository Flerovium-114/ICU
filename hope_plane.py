...
from mavros_msgs.srv import CommandTOL, CommandTOLRequest, CommandLong, CommandBool, CommandBoolRequest, SetMode, SetModeRequest , CommandVtolTransition
...
class drone_api:
    def __init__(self):
        ...
        self.coord_x_prev=0.0
        self.coord_y_prev=0.0
        ...
        self.midpoint_cv = rospy.Subscriber(
            name="{}face_midpoint".format(self.ns),
            data_class=Point,
            queue_size=10,
            callback=self.identify_mp,
        )
        ...
        rospy.wait_for_service("{}mavros/cmd/vtol_transition".format(self.ns))


        self.transition_client = rospy.ServiceProxy(
            name="{}mavros/cmd/vtol_transition".format(self.ns), service_class=CommandVtolTransition
        )
    ...
    def detect_cruise(self):
        if(self.detected==1):
            ...
            offb_set_mode = SetModeRequest()
            offb_set_mode.custom_mode = 'AUTO'
        
            if(self.set_mode_client.call(offb_set_mode).mode_sent == True):
                rospy.loginfo("AUTO enabled")

            vel.twist.linear.x = 10*median_vector[0]
            vel.twist.linear.y = 0
            self.local_vel_pub.publish(vel)


    def identify_mp(self,message):
        self.coord_x_prev=self.coord_x
        self.coord_y_prev=self.coord_y        

        self.coord_x = message.x
        self.coord_y = message.y

        if(self.detect_count>=5):
           self.detect_count=0
           self.detected=1
        elif(self.coord_x==self.coord_x_prev and self.coord_y==self.coord_y_prev):
           self.detected=0
        else:
           self.detected+=1
           

        print("x = ",self.coord_x)
        print("y = ",self.coord_y)
...
if __name__ == '__main__':
    ...
    takeoff_drone()
    print(drone.current_state_g.mode)
    header = Header()
    header.stamp = rospy.Time()
    header.frame_id = "Hope"
    transition_srv = CommandVtolTransition()
    #transition_srv.header=header
    transition_srv.state=4
    drone.transition_client(header,4)
    print(drone.current_state_g.mode)
    
    while True:
        vel.twist.linear.x = 5 * drone.coord_x/(((drone.coord_x)**2 + (drone.coord_y)**2 ))**0.5 
        vel.twist.linear.y = 0
  
        drone.local_vel_pub.publish(vel)
        print(drone.current_state_g.mode)
        drone.detect_cruise()
